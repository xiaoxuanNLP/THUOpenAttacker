...
class TransformersLearnable(Classifier):
    @property
    def TAGS(self):
        if self.__lang_tag is None:
            return super().TAGS
    ...
    def predict(self, sen_list, labels=None):
        ...
        tokeinzed_sen = np.array([
            [self.__tokenizer.cls_token_id] + sen + [self.__tokenizer.sep_token_id] + (
        ...
        for i in range((len(sen_list) + self.batch_size - 1) // self.batch_size):
            ...
            xs = torch.from_numpy(curr_sen).long().to(self.device)
            masks = torch.from_numpy(curr_mask).long().to(self.device)
            outputs = self.model(input_ids=xs, attention_mask=masks, output_hidden_states=True,
    ...
    def get_pred_by_embeds(self,inputs_embeds):
        return self.model(inputs_embeds = inputs_embeds)

    def get_learnable_logits(self, sen_list):
        ...
        for i in range((len(sen_list) + self.batch_size - 1) // self.batch_size):
            ...
            xs = torch.from_numpy(curr_sen).long().to(self.device)
            masks = torch.from_numpy(curr_mask).long().to(self.device)
            output = self.model(input_ids=xs, attention_mask=masks, output_hidden_states=True)
