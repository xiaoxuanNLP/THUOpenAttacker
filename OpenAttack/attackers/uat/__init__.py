...
class UATAttacker(ClassificationAttacker):  # 最没道理的一种攻击方法，直接在句子首/尾加三个单词，然后干预预测，但是这三个词的计算是通过对整个dataset遍历得到的？？？
    @property
    def TAGS(self):
    ...
    def get_triggers(self,
        ...
        requires = [Tag("get_grad", "victim"), Tag("get_prob", "victim"), Tag("get_embedding", "victim"),
                    Tag("get_pred", "victim")]
        for tag in requires:
        ...
        id2word = {v: k for k, v in word2id.items()}  # 通过embedding的word2id得到逆映射id2word
            ...
            for num_iter in tqdm(range((len(dataset) + batch_size - 1) // batch_size), desc="Epoch %d: " % epoch_idx):
                ...
                for i in range(trigger_len):
                    ...
                    for trigger, _ in beams:
                        # map 函数将可迭代对象中的每一个对象依次传入需调用的函数，并返回一个新的可迭代对象。
